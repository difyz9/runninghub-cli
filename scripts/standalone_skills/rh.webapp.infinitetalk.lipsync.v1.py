#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.infinitetalk.lipsync.v1.json.

Skill: rh.webapp.infinitetalk.lipsync.v1
Description: InfiniTetalk emotional talking-head lip-sync video webapp. Uses a portrait image and audio to generate a digital human video.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.infinitetalk.lipsync.v1.py --image_path /path/to/image_path --audio_path /path/to/audio_path
"""
...
SKILL_NAME = 'rh.webapp.infinitetalk.lipsync.v1'
RESOURCE_ID = '2080554801468887041'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'image_path', 'nodeId': '180', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'audio_path', 'nodeId': '6', 'fieldName': 'audio', 'valueMode': 'upload'}]
DEFAULT_INSTANCE_TYPE = 'plus'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.infinitetalk.lipsync.v1_outputs'
INPUTS = {'required': [{'name': 'image_path', 'type': 'image', 'description': 'Local portrait image for the digital human'}, {'name': 'audio_path', 'type': 'audio', 'description': 'Local speech or voice audio file for lip sync'}], 'optional': []}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.infinitetalk.lipsync.v1', description='InfiniTetalk emotional talking-head lip-sync video webapp. Uses a portrait image and audio to generate a digital human video.')
    ...
    p.add_argument('--audio_path', required=True, help='Local speech or voice audio file for lip sync')
    return p
