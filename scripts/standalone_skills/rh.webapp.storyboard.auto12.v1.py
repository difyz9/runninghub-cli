#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.storyboard.auto12.v1.json.

Skill: rh.webapp.storyboard.auto12.v1
Description: Auto storyboard webapp. Uses one reference image and a storyboard prompt to generate a 12-shot storyboard image set.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.storyboard.auto12.v1.py --reference_image_path /path/to/reference_image_path
"""
...
SKILL_NAME = 'rh.webapp.storyboard.auto12.v1'
RESOURCE_ID = '2084512072087465985'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'reference_image_path', 'nodeId': '22', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'storyboard_prompt', 'nodeId': '24', 'fieldName': 'text', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'default'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.storyboard.auto12.v1_outputs'
INPUTS = {'required': [{'name': 'reference_image_path', 'type': 'image', 'description': 'Local reference image path'}], 'optional': [{'name': 'storyboard_prompt', 'type': 'string', 'default': '参考这张图片，帮我生成12张分镜图片，内容是人物日常生活场景，镜头连续、构图丰富、适合短剧分镜。', 'description': 'Storyboard generation prompt'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.storyboard.auto12.v1', description='Auto storyboard webapp. Uses one reference image and a storyboard prompt to generate a 12-shot storyboard image set.')
    ...
    p.add_argument('--reference_image_path', required=True, help='Local reference image path')
    p.add_argument('--storyboard_prompt', default='参考这张图片，帮我生成12张分镜图片，内容是人物日常生活场景，镜头连续、构图丰富、适合短剧分镜。', help='Storyboard generation prompt')
    return p
