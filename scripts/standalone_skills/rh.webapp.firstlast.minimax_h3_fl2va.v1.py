#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.firstlast.minimax_h3_fl2va.v1.json.

Skill: rh.webapp.firstlast.minimax_h3_fl2va.v1
Description: MiniMax H3 FL2VA first-last frame video generation webapp. Uses start/end images with ratio, duration, and prompt.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.firstlast.minimax_h3_fl2va.v1.py --first_frame_image_path /path/to/first_frame_image_path --last_frame_image_path /path/to/last_frame_image_path
"""
...
SKILL_NAME = 'rh.webapp.firstlast.minimax_h3_fl2va.v1'
RESOURCE_ID = '2086547118222827522'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'first_frame_image_path', 'nodeId': '4', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'last_frame_image_path', 'nodeId': '6', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'aspect_ratio', 'nodeId': '7', 'fieldName': 'aspect_ratio', 'valueMode': 'literal'}, {'param': 'duration_seconds', 'nodeId': '7', 'fieldName': 'duration_seconds', 'valueMode': 'literal'}, {'param': 'prompt_text', 'nodeId': '8', 'fieldName': 'prompt', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'default'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.firstlast.minimax_h3_fl2va.v1_outputs'
INPUTS = {'required': [{'name': 'first_frame_image_path', 'type': 'image', 'description': 'Local path for first frame image'}, {'name': 'last_frame_image_path', 'type': 'image', 'description': 'Local path for last frame image'}], 'optional': [{'name': 'aspect_ratio', 'type': 'string', 'default': '16:9', 'description': 'Video aspect ratio: auto, 21:9, 16:9, 4:3, 1:1, 3:4, 9:16'}, {'name': 'duration_seconds', 'type': 'float', 'default': 6, 'description': 'Video duration in seconds (4-15)'}, {'name': 'prompt_text', 'type': 'string', 'default': '保持人物一致性，以图1作为首帧、图2作为尾帧，生成6秒自然过渡视频，动作平滑，镜头稳定，光线统一。', 'description': 'Prompt for first-last frame transition behavior'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.firstlast.minimax_h3_fl2va.v1', description='MiniMax H3 FL2VA first-last frame video generation webapp. Uses start/end images with ratio, duration, and prompt.')
    ...
    p.add_argument('--first_frame_image_path', required=True, help='Local path for first frame image')
    p.add_argument('--last_frame_image_path', required=True, help='Local path for last frame image')
    p.add_argument('--aspect_ratio', default='16:9', help='Video aspect ratio: auto, 21:9, 16:9, 4:3, 1:1, 3:4, 9:16')
    p.add_argument('--duration_seconds', default=6, type=float, help='Video duration in seconds (4-15)')
    p.add_argument('--prompt_text', default='保持人物一致性，以图1作为首帧、图2作为尾帧，生成6秒自然过渡视频，动作平滑，镜头稳定，光线统一。', help='Prompt for first-last frame transition behavior')
    return p
