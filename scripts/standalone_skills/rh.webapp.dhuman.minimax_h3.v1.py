#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.dhuman.minimax_h3.v1.json.

Skill: rh.webapp.dhuman.minimax_h3.v1
Description: MiniMax H3 digital human video generation webapp. Uses a portrait image and speech/action prompt to generate a presenter video.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.dhuman.minimax_h3.v1.py --image_path /path/to/image_path
"""
...
SKILL_NAME = 'rh.webapp.dhuman.minimax_h3.v1'
RESOURCE_ID = '2084829861046214657'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'image_path', 'nodeId': '25', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'prompt_text', 'nodeId': '46', 'fieldName': 'text', 'valueMode': 'literal'}, {'param': 'width', 'nodeId': '47', 'fieldName': 'value', 'valueMode': 'literal'}, {'param': 'height', 'nodeId': '48', 'fieldName': 'value', 'valueMode': 'literal'}, {'param': 'start_time', 'nodeId': '51', 'fieldName': 'value', 'valueMode': 'literal'}, {'param': 'end_time', 'nodeId': '52', 'fieldName': 'value', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'default'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.dhuman.minimax_h3.v1_outputs'
INPUTS = {'required': [{'name': 'image_path', 'type': 'image', 'description': 'Local portrait image for the digital human'}], 'optional': [{'name': 'prompt_text', 'type': 'string', 'default': '图1的人物在干净的演播室中面对镜头，自然清晰地说话，表情和动作协调。', 'description': 'Speech, expression, and action prompt'}, {'name': 'width', 'type': 'int', 'default': 1280, 'description': 'Output width'}, {'name': 'height', 'type': 'int', 'default': 720, 'description': 'Output height'}, {'name': 'start_time', 'type': 'int', 'default': 4, 'description': 'Generation start time in seconds'}, {'name': 'end_time', 'type': 'int', 'default': 10, 'description': 'Generation end time in seconds'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.dhuman.minimax_h3.v1', description='MiniMax H3 digital human video generation webapp. Uses a portrait image and speech/action prompt to generate a presenter video.')
    ...
    p.add_argument('--prompt_text', default='图1的人物在干净的演播室中面对镜头，自然清晰地说话，表情和动作协调。', help='Speech, expression, and action prompt')
    p.add_argument('--width', default=1280, type=int, help='Output width')
    p.add_argument('--height', default=720, type=int, help='Output height')
    p.add_argument('--start_time', default=4, type=int, help='Generation start time in seconds')
    p.add_argument('--end_time', default=10, type=int, help='Generation end time in seconds')
    return p
