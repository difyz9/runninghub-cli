#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.txt2img.zimage_art_portrait.v1.json.

Skill: rh.webapp.txt2img.zimage_art_portrait.v1
Description: Z-image 4K art portrait photography webapp. Single text prompt. Default style is Rembrandt-lit dreamy film photography; rewriting the prompt can produce character three-view (front/side/back) sheets.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.txt2img.zimage_art_portrait.v1.py --prompt_text 'prompt_text_value'
"""
...
SKILL_NAME = 'rh.webapp.txt2img.zimage_art_portrait.v1'
RESOURCE_ID = '2081554936466329602'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'prompt_text', 'nodeId': '53', 'fieldName': 'text', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'default'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.txt2img.zimage_art_portrait.v1_outputs'
INPUTS = {'required': [{'name': 'prompt_text', 'type': 'string', 'description': 'Full text prompt. Replaces the default art-portrait style. For a three-view character sheet, explicitly request 三视图/正面/侧面/背面/并排/白底.'}], 'optional': []}
INPUT_TYPES = {item["name"]: item.get("type", "string") for item in INPUTS.get("required", []) + INPUTS.get("optional", [])}
DOWNLOAD_POLICY = 'keep_smallest'
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.txt2img.zimage_art_portrait.v1', description='Z-image 4K art portrait photography webapp. Single text prompt. Default style is Rembrandt-lit dreamy film photography; rewriting the prompt can produce character three-view (front/side/back) sheets.')
    ...
    p.add_argument('--prompt_text', required=True, help='Full text prompt. Replaces the default art-portrait style. For a three-view character sheet, explicitly request 三视图/正面/侧面/背面/并排/白底.')
    return p
