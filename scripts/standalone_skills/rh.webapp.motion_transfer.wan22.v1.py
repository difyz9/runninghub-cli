#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.motion_transfer.wan22.v1.json.

Skill: rh.webapp.motion_transfer.wan22.v1
Description: WAN2.2 action transfer webapp. Uses a reference image and a motion video to generate action transfer outputs.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.motion_transfer.wan22.v1.py --reference_image_path /path/to/reference_image_path --motion_video_path /path/to/motion_video_path
"""
...
SKILL_NAME = 'rh.webapp.motion_transfer.wan22.v1'
RESOURCE_ID = '2087776618822594562'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'reference_image_path', 'nodeId': '57', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'motion_video_path', 'nodeId': '63', 'fieldName': 'video', 'valueMode': 'upload'}]
DEFAULT_INSTANCE_TYPE = 'plus'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.motion_transfer.wan22.v1_outputs'
INPUTS = {'required': [{'name': 'reference_image_path', 'type': 'image', 'description': 'Local image path used as reference identity/style'}, {'name': 'motion_video_path', 'type': 'video', 'description': 'Local video path used as motion source'}], 'optional': []}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.motion_transfer.wan22.v1', description='WAN2.2 action transfer webapp. Uses a reference image and a motion video to generate action transfer outputs.')
    ...
    p.add_argument('--reference_image_path', required=True, help='Local image path used as reference identity/style')
    p.add_argument('--motion_video_path', required=True, help='Local video path used as motion source')
    return p
