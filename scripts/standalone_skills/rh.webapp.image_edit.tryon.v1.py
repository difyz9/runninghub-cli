#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.image_edit.tryon.v1.json.

Skill: rh.webapp.image_edit.tryon.v1
Description: AI image editing and virtual try-on webapp. Uses a source image and a garment image, then applies prompt-guided editing.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.image_edit.tryon.v1.py --source_image_path /path/to/source_image_path --garment_image_path /path/to/garment_image_path
"""
...
SKILL_NAME = 'rh.webapp.image_edit.tryon.v1'
RESOURCE_ID = '2086630550139392001'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'source_image_path', 'nodeId': '570', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'garment_image_path', 'nodeId': '548', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'prompt_text', 'nodeId': '558', 'fieldName': 'text', 'valueMode': 'literal'}, {'param': 'max_edge', 'nodeId': '539', 'fieldName': 'value', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'default'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.image_edit.tryon.v1_outputs'
INPUTS = {'required': [{'name': 'source_image_path', 'type': 'image', 'description': 'Local source portrait/image path'}, {'name': 'garment_image_path', 'type': 'image', 'description': 'Local garment reference image path'}], 'optional': [{'name': 'prompt_text', 'type': 'string', 'default': '保持人物一致性，图1的模特穿着图2的服装，保持图1的人物外貌、发型的一致性不发生变化，且保持图像的清晰度', 'description': 'Prompt for try-on consistency and quality'}, {'name': 'max_edge', 'type': 'int', 'default': 1280, 'description': 'Longest edge size for output'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.image_edit.tryon.v1', description='AI image editing and virtual try-on webapp. Uses a source image and a garment image, then applies prompt-guided editing.')
    ...
    p.add_argument('--source_image_path', required=True, help='Local source portrait/image path')
    p.add_argument('--garment_image_path', required=True, help='Local garment reference image path')
    p.add_argument('--prompt_text', default='保持人物一致性，图1的模特穿着图2的服装，保持图1的人物外貌、发型的一致性不发生变化，且保持图像的清晰度', help='Prompt for try-on consistency and quality')
    p.add_argument('--max_edge', default=1280, type=int, help='Longest edge size for output')
    return p
