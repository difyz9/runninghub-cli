#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.expression.dhuman.v1.json.

Skill: rh.webapp.expression.dhuman.v1
Description: Digital human facial expression control video webapp. Uses a portrait image to generate a video with a selected expression.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.expression.dhuman.v1.py --image_path /path/to/image_path
"""
...
SKILL_NAME = 'rh.webapp.expression.dhuman.v1'
RESOURCE_ID = '2081607154108289025'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'image_path', 'nodeId': '167', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'expression', 'nodeId': '390', 'fieldName': 'text', 'valueMode': 'literal'}, {'param': 'intensity', 'nodeId': '285', 'fieldName': 'value', 'valueMode': 'literal'}, {'param': 'duration_seconds', 'nodeId': '291', 'fieldName': 'value', 'valueMode': 'literal'}, {'param': 'max_edge', 'nodeId': '292', 'fieldName': 'value', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'standard'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.expression.dhuman.v1_outputs'
INPUTS = {'required': [{'name': 'image_path', 'type': 'image', 'description': 'Local portrait image for the digital human'}], 'optional': [{'name': 'expression', 'type': 'string', 'default': '2.哈哈大笑', 'description': 'Expression instruction: 1.放声大哭, 2.哈哈大笑, 3.害羞微笑, 4.低声哭泣, 5.惊讶错愕, 6.恐惧害怕, 7.紧张局促, 8.爆发怒吼, 9.疲惫困顿, 10.无奈叹气, or custom expression'}, {'name': 'intensity', 'type': 'float', 'default': 30, 'description': 'Expression intensity'}, {'name': 'duration_seconds', 'type': 'int', 'default': 8, 'description': 'Output video duration in seconds'}, {'name': 'max_edge', 'type': 'int', 'default': 1024, 'description': 'Maximum output image edge'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.expression.dhuman.v1', description='Digital human facial expression control video webapp. Uses a portrait image to generate a video with a selected expression.')
    ...
    p.add_argument('--image_path', required=True, help='Local portrait image for the digital human')
    p.add_argument('--expression', default='2.哈哈大笑', help='Expression instruction: 1.放声大哭, 2.哈哈大笑, 3.害羞微笑, 4.低声哭泣, 5.惊讶错愕, 6.恐惧害怕, 7.紧张局促, 8.爆发怒吼, 9.疲惫困顿, 10.无奈叹气, or custom expression')
    p.add_argument('--intensity', default=30, type=float, help='Expression intensity')
    p.add_argument('--duration_seconds', default=8, type=int, help='Output video duration in seconds')
    p.add_argument('--max_edge', default=1024, type=int, help='Maximum output image edge')
    return p
