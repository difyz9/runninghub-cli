#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.music.minimax_music3.v1.json.

Skill: rh.webapp.music.minimax_music3.v1
Description: MiniMax Music3 text-to-song webapp. Uses style metadata and lyric text to generate an MP3 song.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.music.minimax_music3.v1.py --style_text 'style_text_value' --lyrics_text 'lyrics_text_value'
"""
...
SKILL_NAME = 'rh.webapp.music.minimax_music3.v1'
RESOURCE_ID = '2088378137674604546'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'style_text', 'nodeId': '52', 'fieldName': 'text', 'valueMode': 'literal'}, {'param': 'lyrics_text', 'nodeId': '53', 'fieldName': 'text', 'valueMode': 'literal'}, {'param': 'reserved_text', 'nodeId': '888', 'fieldName': 'text', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'default'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.music.minimax_music3.v1_outputs'
INPUTS = {'required': [{'name': 'style_text', 'type': 'string', 'description': 'Song style and arrangement metadata text'}, {'name': 'lyrics_text', 'type': 'string', 'description': 'Song lyric body text'}], 'optional': [{'name': 'reserved_text', 'type': 'string', 'default': '这里不用输入任何提示词，不需要修改', 'description': 'Reserved text node that normally remains unchanged'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.music.minimax_music3.v1', description='MiniMax Music3 text-to-song webapp. Uses style metadata and lyric text to generate an MP3 song.')
    ...
    p.add_argument('--style_text', required=True, help='Song style and arrangement metadata text')
    p.add_argument('--lyrics_text', required=True, help='Song lyric body text')
    p.add_argument('--reserved_text', default='这里不用输入任何提示词，不需要修改', help='Reserved text node that normally remains unchanged')
    return p
