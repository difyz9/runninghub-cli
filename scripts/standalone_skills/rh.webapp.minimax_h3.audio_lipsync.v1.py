#!/usr/bin/env python3
"""Auto-generated standalone script from rh.webapp.minimax_h3.audio_lipsync.v1.json.

Skill: rh.webapp.minimax_h3.audio_lipsync.v1
Description: Audio-driven MiniMax H3 digital human speaking and singing lip-sync video webapp.

Dependencies:
  pip install runninghub-sdk

Usage example:
  python rh.webapp.minimax_h3.audio_lipsync.v1.py --image_path /path/to/image_path --audio_path /path/to/audio_path
"""
...
SKILL_NAME = 'rh.webapp.minimax_h3.audio_lipsync.v1'
RESOURCE_ID = '2085189427940384769'
RESOURCE_TYPE = 'webapp'
MAPPING = [{'param': 'image_path', 'nodeId': '209', 'fieldName': 'image', 'valueMode': 'upload'}, {'param': 'audio_path', 'nodeId': '215', 'fieldName': 'audio', 'valueMode': 'upload'}, {'param': 'prompt_text', 'nodeId': '214', 'fieldName': 'value', 'valueMode': 'literal'}]
DEFAULT_INSTANCE_TYPE = 'standard'
DEFAULT_TIMEOUT = 1800
DEFAULT_POLL_INTERVAL = 15
DEFAULT_OUTPUT_SUBDIR = 'rh.webapp.minimax_h3.audio_lipsync.v1_outputs'
INPUTS = {'required': [{'name': 'image_path', 'type': 'image', 'description': 'Local portrait image for the digital human'}, {'name': 'audio_path', 'type': 'audio', 'description': 'Local speech or singing audio file'}], 'optional': [{'name': 'prompt_text', 'type': 'string', 'default': '<image0>角色在深情的唱歌。', 'description': 'Action or singing prompt; use <image0> to reference the uploaded image'}]}
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog='rh-rh.webapp.minimax_h3.audio_lipsync.v1', description='Audio-driven MiniMax H3 digital human speaking and singing lip-sync video webapp.')
    ...
    p.add_argument('--audio_path', required=True, help='Local speech or singing audio file')
    p.add_argument('--prompt_text', default='<image0>角色在深情的唱歌。', help='Action or singing prompt; use <image0> to reference the uploaded image')
    return p
