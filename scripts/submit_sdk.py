...
PROMPTS = [
    ...
    "In the style of E.H. Shepard, fine ink illustration with watercolor. An open ancient Chinese book on a wooden desk. One page shows the handwritten characters Pian Mu in elegant brushstrokes, the other page has a delicate drawing of a six-fingered hand. Soft candlelight illuminates the pages. Warm sepia and parchment tones. Quiet scholarly atmosphere.",
    ...
    "In the style of E.H. Shepard, elegant pen and ink with soft wash. The Chinese characters Dang Xia written in flowing calligraphy, but rendered with a fine nib like E.H. Shepard's hand. Soft warm golden watercolor glow behind the characters. Simple, uncluttered. Meditative quality. Ink splatters like tiny birds in the margins.",
...
print(f"开始提交 {total} 个文生图任务（2并发，SDK 直连）")
print("=" * 50)
...
while done < total:
    ...
    while len(active) < 2 and done < total:
        ...
        try:
            modifier = NodeModifier().text("70", prompt)
            task = client.run_ai_app_with_modifier(
                webapp_id=APP_ID, modifier=modifier, instance_type="default"
            )
            tid = task.task_id
            active[tid] = label
            results.append(f"{label}:{tid}")
            print(f"✓ {tid}")
            done += 1
    ...
    for tid, lbl in active.items():
        try:
            ...
            print(f"  {lbl}: {st.value}")
            if st in (TaskStatus.SUCCESS, TaskStatus.FAILED):
                finished.append(tid)
        except Exception as e:
            print(f"  {lbl}: 查询失败 {str(e)[:50]}")
    for tid in finished:
        del active[tid]

print("=" * 50)
print(f"✅ 全部提交完成: {len(results)}/{total}")
print(f"📄 {TASK_FILE}")
